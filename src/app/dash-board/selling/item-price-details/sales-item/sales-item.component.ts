...
export class SalesItemComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.salesItemManager.allsalesitem().subscribe(response => {
            this.saleItm = deserialize<Itemdt001mb[]>(Itemdt001mb, response);
            if (this.saleItm.length > 0) {
                this.gridOptions?.api?.setRowData(this.saleItm);
            } else {
    ...
    onDeleteButtonClick(params: any) {
        this.salesItemManager.deletesalesitem(params.data.itemid).subscribe((response) => {
            for (let i = 0; i < this.saleItm.length; i++) {
                if (this.saleItm[i].itemid == params.data.itemid) {
                    this.saleItm?.splice(i, 1);
                    break;
                }
            }
    ...
    onOrderClick(event: any, saleForm: any) {
        ...
        if (this.itemid) {
            ...
            this.salesItemManager.updatesalesitem(itemdt001mb).subscribe(response => {
                ...
                for (let saleItem of this.saleItm) {
