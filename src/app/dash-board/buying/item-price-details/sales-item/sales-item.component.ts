...
export class SalesItemComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.salesItemManager.allsalesitem().subscribe(response => {
            this.salesItem = deserialize<Itemdt001mb[]>(Itemdt001mb, response);
            if (this.salesItem.length > 0) {
                this.gridOptions?.api?.setRowData(this.salesItem);
            } else {
    ...
    onDeleteButtonClick(params: any) {
        this.salesItemManager.deletesalesitem(params.data.itemid).subscribe((response) => {
            for (let i = 0; i < this.salesItem.length; i++) {
                if (this.salesItem[i].itemid == params.data.itemid) {
                    this.salesItem?.splice(i, 1);
                    break;
                }
            }
    ...
    onOrderClick(event: any, saleForm: any) {
        ...
        if (this.itemid) {
            ...
            this.salesItemManager.updatesalesitem(itemdt001mb).subscribe(response => {
                ...
                for (let saleItem of this.salesItem) {
    ...
    onGeneratePdfReport(){
		this.salesItemManager.salesItemPdf().subscribe((response) =>{
            saveAs(response,"salesItemDetails");
