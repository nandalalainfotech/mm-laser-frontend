...
export class PurchaseOrderComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.systemPropertiesService
            .system(this.matname, this.mattype)
            .subscribe((response) => {
                this.matsystemproperties = deserialize<Systemproperties001mb[]>(
                    Systemproperties001mb,
                    response
                );
            });
        this.purchaseOrdersManager.allpurchaseorder().subscribe((response) => {
            this.purchaseOrder = deserialize<Prpurchaseord001mb[]>(
                Prpurchaseord001mb,
                response
            );
    ...
    get f() {
        return this.prOrderForm.controls;
    }
    ...
    onDeleteButtonClick(params: any) {
        this.purchaseOrdersManager
            .purchaseorderdelete(params.data.proId)
            .subscribe((response) => {
    ...
    onOrderClick(event: any, prOrderForm: any) {
        ...
        prpurchaseord001mb.poSeries = this.f.poSeries.value
            ? this.f.poSeries.value
            : '';
        prpurchaseord001mb.date = new Date(this.f.date.value);
        prpurchaseord001mb.supplier = this.f.supplier.value
            ? this.f.supplier.value
            : '';
        prpurchaseord001mb.supplyrawmat = this.f.supplyrawmat.value
            ? this.f.supplyrawmat.value
            : '';
        prpurchaseord001mb.itemCode = this.f.itemCode.value
            ? this.f.itemCode.value
            : '';
        prpurchaseord001mb.quantity = this.f.quantity.value
            ? this.f.quantity.value
            : '';
        prpurchaseord001mb.rate = this.f.rate.value ? this.f.rate.value : '';
        prpurchaseord001mb.amount = this.f.amount.value
            ? this.f.amount.value
            : '';
        prpurchaseord001mb.prtype = this.f.prtype.value
            ? this.f.prtype.value
            : '';
        prpurchaseord001mb.accountHead = this.f.accountHead.value
            ? this.f.accountHead.value
            : '';
        prpurchaseord001mb.quotrate = this.f.quotrate.value
            ? this.f.quotrate.value
            : '';
        prpurchaseord001mb.taxandChg = this.f.taxandChg.value
            ? this.f.taxandChg.value
            : '';
        if (this.proId) {
            prpurchaseord001mb.proId = parseInt(this.proId);
            prpurchaseord001mb.insertUser = this.insertUser;
            prpurchaseord001mb.insertDatetime = this.insertDatetime;
            prpurchaseord001mb.updatedUser =
                this.authManager.getcurrentUser.username;
            prpurchaseord001mb.updatedDatetime = new Date();
            this.purchaseOrdersManager
                .purchaseorderupdate(prpurchaseord001mb)
                .subscribe((response) => {
                    this.calloutService.showSuccess(
                        'Order Updated Successfully'
                    );
                    let proder = deserialize<Prpurchaseord001mb>(
                        Prpurchaseord001mb,
                        response
                    );
            ...
            prpurchaseord001mb.insertUser =
                this.authManager.getcurrentUser.username;
            prpurchaseord001mb.insertDatetime = new Date();
            this.purchaseOrdersManager
                .purchaseordersave(prpurchaseord001mb)
                .subscribe((response) => {
